        ...
        try {
            if (mappingUrls.containsKey(url)) {
                ...
                try (PrintWriter out = res.getWriter()) {
                    out.println(
                            "<!DOCTYPE html><html><head><style>body{font-family:Arial,sans-serif;margin:40px;background:#f5f5f5;color:#333}pre{background:#fff;padding:15px;border-radius:5px;border:1px solid #ddd}</style></head><body>");
                    if (result instanceof String html) {
                        out.println(html);
                    } else {
                        out.println("<pre>" + url + " -> " + mapping.getClassName() + " -> " + mapping.getMethod()
                                + "()</pre>");
                    }
                ...
                try (PrintWriter out = res.getWriter()) {
                    out.println(
                            "<!DOCTYPE html><html><head><style>body{font-family:Arial,sans-serif;margin:40px;background:#f5f5f5;color:#333}h1{color:#333}ul{background:#fff;padding:20px;border-radius:5px;border:1px solid #ddd}li{margin:5px 0}</style></head><body>");
                    out.println("<h1>URL non trouvee : " + url + "</h1>");
                    out.println("<h2>Routes disponibles :</h2><ul>");
                    for (Map.Entry<String, Mapping> entry : mappingUrls.entrySet()) {
                        Mapping m = entry.getValue();
                        out.println("<li>" + entry.getKey() + " -> " + m.getClassName() + " -> " + m.getMethod()
                                + "()</li>");
                    }
                    out.println("</ul>");
                    out.println("</body></html>");
